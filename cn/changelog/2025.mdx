...
- **行动：** 启动空投第二季，奖金池为 100 万 $XNY（3 个月线性归属）。
- **新前沿：**
...
- **血统：** 实现链上和链下活动的完整可追溯性和验证，建立不可篡改的数据来源。
...
- **原因：** 该模块已完成当前产品路线图下的计划生命周期。停用此模块有助于系统精简和资源重新配置，以支持即将发布的功能。
- **注意：** 用户在此模块的历史贡献记录和获得的奖励保持不变。此更改不影响其他前沿模块。
...
- **原因：** 此决定是我们持续产品精简的一部分，旨在将资源和用户注意力集中在我们最高优先级的活跃前沿上。机器人前沿已完成其计划的探索阶段，停用此模块使我们能够整合努力。
...
- **原因：** 此决定是我们持续产品精简的一部分，旨在将资源和用户注意力集中在我们最高优先级的活跃前沿上。加密前沿已完成其计划的探索阶段，停用此模块使我们能够整合努力。
...
- **如何：** 选定的前沿任务提供额外的$XNY或USDT奖励，与基础积分无关。活动参数（奖励金额、持续时间、目标）根据前沿设置，并在其各自的主页上显示。
...
通过收集超越标签的丰富注释，深入AI对食物的理解，包括重量、烹饪方法和热量内容。
...
通过收集即食食品的注释图像，教会AI对人类饮食习惯的细致理解，关注文化和情境的相关性，而不仅仅是简单标签。
...
通过对素食、非素食和混合类别进行细致的图像标注，扩展AI对全球饮食文化和偏好的理解。
