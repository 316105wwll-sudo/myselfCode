...
系统性收集物理、金融和多模态领域的学术级数据集，并为三个新前沿建立高质量数据管道。
...
通过高奖励机制和反垃圾邮件执行系统，系统性地收集高质量、结构化的数据，确保有效性和学术价值，并奖励长期贡献者，丰富平台的数据生态系统，推出五个新前沿模块。
...
- **如何：** 选定的前沿任务提供额外的$XNY或USDT奖励，与基础积分无关。活动参数（奖励金额、持续时间、目标）按前沿设置，并在其各自主页上显示。
